 ...
 /**
  ...
  private function mail_passthru($to, $subject, $body, $header, $params) {
    if ( ini_get('safe_mode') || !($this->UseSendmailOptions) ) {
        $rt = @mail($to, $this->EncodeHeader($this->SecureHeader($subject), 'text', 1), $body, $header);
    } else {
        $rt = @mail($to, $this->EncodeHeader($this->SecureHeader($subject), 'text', 1), $body, $header, $params);
    }
    return $rt;
  }
  ...
  public function PreSend() {
    try {
      ...
      if ($this->Mailer == 'mail') {
        ...
        $this->mailHeader .= $this->HeaderLine('Subject', $this->EncodeHeader($this->SecureHeader(trim($this->Subject)), 'text', 1));
        // if(count($this->cc) > 0) {
